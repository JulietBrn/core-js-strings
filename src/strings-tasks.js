...
function repeatString(str, times) {
  return str === '' || times < 1 ? '' : str.repeat(times);
}
...
function countVowels(str) {
  const strLow = str.toLowerCase().replace(/[aeuioy]/gi, '');
  return str.length - strLow.length;
}
...
function reverseWords(str) {
  const arr = str.split(' ');
  const reArr = arr.map((val) => reverseString(val));
  return reArr.join(' ');
}
...
function encodeToRot13(str) {
  ...
  const result = str.split('').map((val) => {
    if (val === ' ' || val === '!' || val === '?') {
      return val;
    }
